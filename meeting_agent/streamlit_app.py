...
def main():
    ...
    with tab3:
        ...
        st.markdown("Generate structured meeting notes from the current transcript using LLM.")

        if not st.session_state.transcript_text:
            st.warning("⚠️ No transcript available. Please upload or transcribe a transcript first.")
            st.stop()

        st.info(f"📄 Current transcript: {st.session_state.transcript_path.name if st.session_state.transcript_path else 'In-memory transcript'} ({len(st.session_state.transcript_text)} characters)")
        ...
        if st.button("🚀 Generate Notes", type="primary"):
            try:
                with st.status("🔄 Generating meeting notes... This may take a moment.", state="running", expanded=True) as status:
                    ...
                    response = agent.generate_meeting_notes(st.session_state.transcript_text)
    ...
    with tab4:
        st.header("Q&A Chat")
        st.markdown("Ask questions about the current transcript using natural language.")
        ...
        if st.session_state.current_transcript_id:
            db = st.session_state.db
            db_chat_history = db.get_chat_history(st.session_state.current_transcript_id)
                ...
                with st.chat_message("assistant"):
                    ...
                    try:
                        ...
                        user_prompt = user_prompt_template.format(
                            question=prompt, transcript=st.session_state.transcript_text
                        )
                        ...
                        if answer:
                            ...
                            if st.session_state.current_transcript_id:
                                db = st.session_state.db
                                db.save_chat_message(st.session_state.current_transcript_id, "user", prompt)
                                db.save_chat_message(st.session_state.current_transcript_id, "assistant", answer)
                        else:
                            ...
                            if st.session_state.current_transcript_id:
                                db = st.session_state.db
                                db.save_chat_message(st.session_state.current_transcript_id, "user", prompt)
                                db.save_chat_message(st.session_state.current_transcript_id, "assistant", error_msg)
                    except Exception as e:
                        # Replace thinking indicator with error
                        thinking_placeholder.empty()
    ...
    with tab5:
        st.header("Downloads")
        st.markdown("Download generated meeting notes in various formats.")

        if not st.session_state.generated_notes:
            st.warning("⚠️ No notes generated yet. Please generate notes in the Generate Notes tab.")
            st.stop()

        st.info("📄 Generated notes are available for download in multiple formats.")
    ...
    with tab6:
        ...
        st.markdown("Analyze the sentiment of the transcript and individual attendees.")
        
        # #region agent log
        try:
            log_file = open("/Users/ymo/meeting_agent/.cursor/debug.log", "a", encoding="utf-8")
            log_file.write(json.dumps({"location": "streamlit_app.py:640", "message": "After header, checking transcript", "data": {"has_transcript": st.session_state.transcript_text is not None, "has_results": st.session_state.sentiment_results is not None, "has_error": st.session_state.sentiment_error is not None}, "timestamp": time.time() * 1000, "sessionId": "debug-session", "runId": "run1", "hypothesisId": "D"}) + "\n")
            log_file.flush()
            log_file.close()
        except Exception as log_err:
            st.error(f"DEBUG: Log error 2: {log_err}")
        # #endregion
        ...
        if not st.session_state.transcript_text:
            ...
            try:
                log_file = open("/Users/ymo/meeting_agent/.cursor/debug.log", "a", encoding="utf-8")
                log_file.write(json.dumps({"location": "streamlit_app.py:650", "message": "No transcript, stopping", "data": {}, "timestamp": time.time() * 1000, "sessionId": "debug-session", "runId": "run1", "hypothesisId": "D"}) + "\n")
                log_file.flush()
                log_file.close()
            except Exception:
                pass
        ...
        try:
            ...
            with col3:
                st.metric("Text Length", f"{len(st.session_state.transcript_text):,} chars")
            ...
            with gov_tab1:
                ...
                with st.expander("🤖 LLM-Powered Policy Generation", expanded=False):
                    ...
                    analysis_scope = st.radio(
                        ...
                        help="Analyze the current transcript or all transcripts in the database",
                    ...
                    if analysis_scope == "Current Transcript":
                        if st.session_state.current_transcript_id:
                            ...
                            st.warning("⚠️ No transcript currently loaded. Please load a transcript first.")
                                                ...
                                                with col_btn1:
                                                    if st.button(f"✅ Create Policy", key=f"create_policy_{idx}"):
                                                        try:
                                                            ...
                                                            st.success(f"✅ Policy created! (ID: {policy_id})")
                                                            st.rerun()
