...
class Database:
    ...
    def get_transcripts_with_sentiment(self, limit: int | None = None) -> list[dict[str, Any]]:
        """Get transcripts that have sentiment analysis data.

        Args:
            limit: Maximum number of results (optional).

        Returns:
            list: List of transcript dictionaries with sentiment analysis.
        """
        conn = self._get_connection()
        cursor = conn.cursor()

        query = """
            SELECT DISTINCT t.*, sa.analyzed_at, sa.model_used
            FROM transcripts t
            JOIN sentiment_analysis sa ON t.id = sa.transcript_id
            WHERE t.archived = 0
        ...
        for row in rows:
            transcript = dict(row)
            # Convert file_path back to Path object if needed
            if 'file_path' in transcript and transcript['file_path']:
                transcript['file_path'] = Path(transcript['file_path'])
